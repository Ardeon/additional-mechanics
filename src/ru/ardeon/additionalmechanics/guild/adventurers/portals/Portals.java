...
import ru.ardeon.additionalmechanics.util.ItemSerializer;
...
public class Portals {
	...
	public Portals() {
		AdditionalMechanics.getPlugin().getServer().getPluginManager().registerEvents(new Portalslistener(this), AdditionalMechanics.getPlugin());
		AdditionalMechanics.getPlugin().getServer().getPluginCommand("enderbook").setExecutor(new EnderBookCommand(this));
    ...
    public void save(UUID id) {
    	File f = new File(AdditionalMechanics.getPlugin().getDataFolder()+File.separator+"PortalBooks", id.toString()+".json");
    	PortalMenu menu = map.get(id);
		f.getParentFile().mkdirs();
		if (menu==null) {
			AdditionalMechanics.getPlugin().getLogger().info("save error");
			return;
		}
		try(FileWriter writer = new FileWriter(f, false)) {
        	...
        	AdditionalMechanics.getPlugin().getLogger().info(ex.toString());
        } 
    }
	
	
    public void loadAll() {
    	File f = new File(AdditionalMechanics.getPlugin().getDataFolder()+File.separator+"PortalBooks");
    ...
    public PortalMenu getFromFile(File f) {
    	...
    	if (f.exists()) {
			...
			try {
				...
				AdditionalMechanics.getPlugin().getLogger().info("open error");
			} catch (IOException e1) {
				AdditionalMechanics.getPlugin().getLogger().info("error");
			}
